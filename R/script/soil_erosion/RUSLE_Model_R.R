...
calculateR <- function(input_wd, rf_st_date, rf_en_date, aoi){
    setwd(input_wd)
    woreda <- terra::vect(aoi)
    dates <- c(rf_st_date, rf_en_date)
    datevec <-
      seq(as.Date(dates[1]),
          by = "day",
          length.out = difftime(rf_en_date, rf_st_date, 'days') + 1)
    rf <-
      get_chirps(
        woreda,
    ...
    # calculates the mean for each year and sums the whole years
